import { usePublishedApps, useUnpublishApp } from "@/hooks/use-apps";
import { Link } from "wouter";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Globe, Share2, Lock, Trash2, ExternalLink, Calendar } from "lucide-react";
import { format } from "date-fns";
import { useToast } from "@/hooks/use-toast";

export default function PublishedApps() {
  const { data: apps, isLoading } = usePublishedApps();
  const unpublishApp = useUnpublishApp();
  const { toast } = useToast();

  if (isLoading) {
    return (
      <div className="container mx-auto px-4 py-8 max-w-7xl animate-pulse space-y-8">
        <div className="h-10 w-48 bg-muted rounded-lg" />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-64 bg-muted rounded-2xl" />
          ))}
        </div>
      </div>
    );
  }

  const publishedApps = apps?.filter(app => app.publicLink) ?? [];

  return (
    <div className="container mx-auto px-4 py-12 max-w-7xl">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-10 gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-foreground">Published Apps</h1>
          <p className="text-muted-foreground mt-1">Your publicly accessible applications</p>
        </div>
      </div>

      {publishedApps.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 bg-card border border-dashed border-border rounded-3xl text-center">
          <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center text-primary mb-6">
            <Globe className="w-8 h-8" />
          </div>
          <h2 className="text-xl font-bold mb-2">No published apps yet</h2>
          <p className="text-muted-foreground mb-8 max-w-sm">
            Publish your apps from the Dashboard to make them accessible via public links.
          </p>
          <Link href="/">
            <Button className="gap-2">
              <Globe className="w-4 h-4" />
              Go to Dashboard
            </Button>
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {publishedApps.map((app) => (
            <Card key={app.id} className="group hover:shadow-lg transition-all duration-300 border-border/60 hover:border-primary/50">
              <CardHeader className="relative">
                <div className="flex justify-between items-start">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-primary/10 to-primary/5 flex items-center justify-center text-primary mb-2 group-hover:scale-110 transition-transform duration-300">
                    <Globe className="w-6 h-6" />
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => {
                        const link = `${window.location.origin}/${app.publicLink}`;
                        navigator.clipboard.writeText(link);
                        toast({ title: "Link copiado", description: "Link público copiado para a área de transferência" });
                      }}
                      className="h-8 w-8"
                    >
                      <Share2 className="w-4 h-4" />
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => window.open(`/${app.publicLink}`, "_blank")}
                      className="h-8 w-8"
                    >
                      <ExternalLink className="w-4 h-4" />
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => unpublishApp.mutate(app.id)}
                      disabled={unpublishApp.isPending}
                      className="h-8 w-8 text-destructive hover:text-destructive"
                    >
                      <Lock className="w-4 h-4" />
                    </Button>
                  </div>
                </div>
                <CardTitle className="text-xl">{app.name ?? "Untitled App"}</CardTitle>
                <CardDescription className="flex items-center gap-1 mt-1">
                  <Calendar className="w-3.5 h-3.5" />
                  Published {format(new Date(app.updatedAt), "MMM d, yyyy")}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="p-3 bg-muted/50 rounded-lg">
                  <p className="text-xs text-muted-foreground mb-1">Public Link</p>
                  <div className="flex items-center gap-2">
                    <code className="flex-1 text-sm font-mono bg-background px-2 py-1 rounded border break-all">
                      {window.location.origin}/{app.publicLink}
                    </code>
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => {
                        const link = `${window.location.origin}/${app.publicLink}`;
                        navigator.clipboard.writeText(link);
                        toast({ title: "Link copiado", description: "Link público copiado para a área de transferência" });
                      }}
                      className="h-8"
                    >
                      <Share2 className="w-4 h-4" />
                    </Button>
                  </div>
                </div>
                <div className="flex gap-2 pt-2 border-t">
                  <Link href={`/app/${app.id}/editor`} className="flex-1">
                    <Button variant="outline" className="w-full gap-2">
                      <ExternalLink className="w-4 h-4" />
                      Editor
                    </Button>
                  </Link>
                  <Link href={`/app/${app.id}/preview`} className="flex-1">
                    <Button className="w-full gap-2 btn-primary">
                      <Globe className="w-4 h-4" />
                      Preview
                    </Button>
                  </Link>
                </div>
              </CardContent>
            </Card>
          ))}
        </div>
      )}
    </div>
  );
}